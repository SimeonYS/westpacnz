...
class WwestpacnzPipeline:

    # Database setup
    conn = sqlite3.connect('westpacnz.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `westpacnz`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM westpacnz WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO westpacnz (date, title, link, content)"
